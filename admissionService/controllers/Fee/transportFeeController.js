const { transportfeecollection , transportfeecollectiondetail, StudentFeeGroupDetailPrice, sequelize } = require('../../models');
...
exports.createStudentFeeCollection = async (req, res) => {
  ...
  try {
    ...
    const createdFeeCollection = await transportfeecollection.create(feecollection, { transaction });
    ...
    const createdMonthly = await transportfeecollectiondetail.bulkCreate(sanitizedMonthly, { transaction });
    ...
    return res.status(201).json({
      ...
      data: {
        transportfeecollection: createdFeeCollection,
        fee_table_id,
        transportfeecollectiondetail_created_count: createdMonthly.length,
