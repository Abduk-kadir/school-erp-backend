const {admissionfeecollection , admissionfeecollectiondetail, StudentFeeGroupDetailPrice, sequelize, } = require('../../models');
...
exports.createStudentFeeCollection = asyncHandler(async (req, res) => {
  ...
  const result = await sequelize.transaction(async (transaction) => {
    const createdFeeCollection = await admissionfeecollection.create(feecollection, { transaction });
    ...
    const createdMonthly = await admissionfeecollectiondetail.bulkCreate(sanitizedMonthly, { transaction });
  ...
  return res.status(201).json({
    ...
    data: {
      admissionfeecollection: result.createdFeeCollection,
      fee_table_id: result.fee_table_id,
      admissionfeecollectiondetail_created_count: result.createdMonthlyCount,
...
exports.getAllFees = asyncHandler(async (req, res) => {
  ...
  const query = `select fc.*, p.first_name,p.class,p.division, c.class_name as className from admissionfeecollections as fc join par_student_personal_informations as p on fc.reg_no=p.reg_no
