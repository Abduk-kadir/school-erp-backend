...
module.exports = {
  async up(queryInterface, Sequelize) {
    ...
    try {
      await queryInterface.describeTable('transportfeecollectiondetails');
    } catch (e) {
      tableExists = false;
    }

    if (!tableExists) {
      await queryInterface.createTable('transportfeecollectiondetails', {
        id: {
          allowNull: false,
          autoIncrement: true,
          primaryKey: true,
    ...
    const indexes = await queryInterface.showIndex('transportfeecollectiondetails');
    const hasFeeTableIdIndex = indexes.some(
      (idx) =>
        idx.name === 'transportfeecollectiondetails_fee_table_id_idx' ||
    ...
    if (!hasFeeTableIdIndex) {
      await queryInterface.addIndex('transportfeecollectiondetails', ['fee_table_id'], {
        name: 'transportfeecollectiondetails_fee_table_id_idx',
      });
    }
  },

  async down(queryInterface) {
    await queryInterface.dropTable('transportfeecollectiondetails');
  },
};
