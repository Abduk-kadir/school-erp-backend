...
module.exports = {
  async up(queryInterface, Sequelize) {
    ...
    try {
      await queryInterface.describeTable('canteefeecollectiondetails');
    } catch (e) {
      tableExists = false;
    }

    if (!tableExists) {
      await queryInterface.createTable('canteefeecollectiondetails', {
        id: {
          allowNull: false,
          autoIncrement: true,
          primaryKey: true,
    ...
    const indexes = await queryInterface.showIndex('canteefeecollectiondetails');
    const hasFeeTableIdIndex = indexes.some(
      (idx) =>
        idx.name === 'canteefeecollectiondetails_fee_table_id_idx' ||
    ...
    if (!hasFeeTableIdIndex) {
      await queryInterface.addIndex('canteefeecollectiondetails', ['fee_table_id'], {
        name: 'canteefeecollectiondetails_fee_table_id_idx',
      });
    }
  },

  async down(queryInterface) {
    await queryInterface.dropTable('canteefeecollectiondetails');
  },
};
